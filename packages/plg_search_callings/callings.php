...
/**
 * Callings Search plugin
 *
 * @package     Joomla.Plugin
 * @subpackage  Search.callings
 * @since       3.0
 */
class PlgSearchCallings extends JPlugin
{
	/**
	 * Constructor
	 *
	...
	{
		static $areas = array(
			'callings' => 'PLG_SEARCH_CALLINGS_CALLINGS'
		);
	...
	public function onContentSearch($text, $phrase='', $ordering='', $areas=null)
	{
		...
		$section = JText::_('PLG_SEARCH_CALLINGS');
		$wheres	= array();
		...
		if (!empty($state))
		{
			...
			if ($rows)
			{
				...
				{
					$rows[$key]->href = CallingsHelperRoute::getCallingRoute($row->slug);
				}

				foreach ($rows as $key => $calling)
				{
					if (searchHelper::checkNoHTML($calling, $searchText, array('text', 'title')))
					{
						$return[] = $calling;
					}
				}
