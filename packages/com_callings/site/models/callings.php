...
/**
 * This models supports retrieving lists of callings.
 *
 * @package     Callings
 * @subpackage  com_callings
 * @since       3.0
 */
class CallingsModelCallings extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		// Load the status.
		$status = $input->get('status', null, 'uint');
	...
	protected function getListQuery()
	{
		...
		if ($status === 1)
		{
			$query->where('a.date_closing >= "' . JFactory::getDate()->format("Y-m-d") . '"');
		}
		elseif ($status === 0)
		{
			$query->where('a.date_closing < "' . JFactory::getDate()->format("Y-m-d") . '"');
